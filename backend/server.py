...
# Background scheduler (subscription expiry + contract reminders)
...
# Admin trigger endpoint (manual run for testing or immediate refresh)
@app.post("/api/admin/cron/subscription-check")
...
@app.post("/api/admin/cron/contract-check")
...
async def on_startup():
    ...
    # Schedule subscription expiry cron — runs hourly
    scheduler.add_job(check_subscription_expiry_job, "interval", hours=1, id="subscription_expiry", replace_existing=True, next_run_time=datetime.now(timezone.utc) + timedelta(seconds=30))
    # Schedule contract expiry reminder — runs daily at 09:00 Asia/Jakarta (UTC+7)
    scheduler.add_job(check_contract_expiry_job, "cron", hour=2, minute=0, id="contract_expiry_reminder", replace_existing=True)
    scheduler.start()
