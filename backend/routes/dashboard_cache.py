...
def reset() -> None:
    _cache.update({"total_eur": 0, "total_idr": 0, "computed_at": 0.0, "refreshing": False})
...
async def recompute() -> Dict[str, Any]:
    ...
    try:
        ...
        _cache.update({"total_eur": total_eur, "total_idr": total_idr, "computed_at": computed_at})
        try:
            await db_bg.metrics_cache.update_one(
                {"_id": "dashboard_revenue"},
                {"$set": {"total_eur": total_eur, "total_idr": total_idr, "computed_at": computed_at}},
                upsert=True,
            )
