...
async def seed_indexes_and_admins():
    ...
    await db_bg.payments.create_index([("status", 1), ("paid_at", -1)])
    ...
    await db_bg.royalty_import_replacement_rows.create_index([("job_id", 1), ("projected_balance_delta_idr", -1)])
    replacement_indexes = await db_bg.royalty_import_replacements.index_information()
    if "replacement_import_id_1" not in replacement_indexes:
        await db_bg.royalty_import_replacements.create_index("replacement_import_id", unique=True, sparse=True)
    ...
    await db_bg.balance_transactions.create_index([("label_id", 1), ("type", 1), ("status", 1), ("created_at", -1)])
    await db_bg.withdraw_requests.create_index([("label_id", 1), ("status", 1), ("adjustment_ids", 1)])
    await db_bg.bank_accounts.create_index("label_id", unique=True)
    await db_bg.bank_account_change_requests.create_index([("label_id", 1), ("created_at", -1)])
    ...
    await db_bg.withdraw_requests.create_index([("status", 1), ("request_date", -1)])
    await db_bg.withdraw_requests.create_index([("status", 1), ("paid_date", -1)])
    ...
    await db_bg.kyc_documents.create_index("id", unique=True)
    await db_bg.kyc_documents.create_index([("status", 1), ("is_current", 1), ("uploaded_at", 1)])
    await db_bg.kyc_documents.create_index([("label_id", 1), ("is_current", 1)])
    await db_bg.notifications.create_index("event_key", unique=True, sparse=True)
    await db_bg.payment_notification_deliveries.create_index("event_key", unique=True)
