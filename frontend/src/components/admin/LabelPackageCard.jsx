...
export const LabelPackageCard = ({ label, onChanged }) => {
  ...
  return <section className="rm-card min-w-0 space-y-4 p-5" data-testid="admin-label-subscription-card">
    <h2 className="flex items-center gap-2 font-display text-lg font-bold"><Package className="h-4 w-4" />{t("Paket & Langganan")}</h2>
    ...
    <button type="button" className="rm-btn-primary inline-flex items-center gap-2 text-sm" disabled={!valid || saving} onClick={() => setConfirming(true)} data-testid="admin-label-sub-save">{t("Tinjau Perubahan")}<ArrowRight className="h-4 w-4" /></button>
    <Dialog open={confirming} onOpenChange={(open) => { if (!saving) setConfirming(open); }}><DialogContent className="ui-menu max-h-[90dvh] w-[calc(100%-2rem)] max-w-lg overflow-y-auto rounded-lg" data-testid="admin-label-package-confirm-dialog" closeTestId="admin-label-package-confirm-close">
      <DialogHeader><DialogTitle>{t("Konfirmasi Perubahan Paket")}</DialogTitle><DialogDescription>{t("Perubahan langsung berlaku tanpa membuat tagihan otomatis. Invoice yang sudah ada tidak berubah.")}</DialogDescription></DialogHeader>
