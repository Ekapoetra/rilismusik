  ...
  return (
    <div className="absolute inset-0 z-20 flex items-start justify-center px-4 pt-24 md:pt-32" data-testid="label-kyc-lock-overlay">
      <div className="w-full max-w-lg rounded-lg border border-white/10 bg-[#0F0F0F]/95 p-7 shadow-2xl backdrop-blur-xl rm-fade-up" role="dialog" aria-modal="true" data-testid="label-kyc-lock-dialog">
        ...
        <h2 className="mt-2 font-display text-2xl font-extrabold" data-testid="label-kyc-lock-title">{loading ? "Memeriksa aktivasi akun" : pending ? "KYC sedang direview" : rejected ? "KYC perlu diperbaiki" : "Selesaikan aktivasi KYC"}</h2>
        ...
        {!loading && <Link to="/label/profile" className="rm-btn-primary mt-6 inline-flex items-center justify-center" data-testid="label-kyc-lock-profile-link">Buka Profil & KYC</Link>}
      </div>
    </div>
  );
};