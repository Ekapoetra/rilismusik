...
const AdminSidebar = ({ instance, collapsed, onCollapse, onNavigate }) => {
  ...
  return <div className="flex h-full flex-col bg-[#090909]" data-testid={`admin-sidebar-${instance}`}>
    ...
    <div className="space-y-3 border-t border-white/10 p-3"><div className={`flex ${collapsed ? "flex-col" : "items-center justify-between"} gap-2`}><button type="button" onClick={() => setLocale(locale === "id" ? "en" : "id")} className="rounded-md border border-white/10 px-2.5 py-1.5 text-[11px] font-bold uppercase text-zinc-300 hover:bg-white/5" data-testid={`admin-locale-toggle-${instance}`}>{locale === "id" ? "ID" : "EN"}</button><NotificationBell /></div>{!collapsed && <div><div className="truncate text-xs font-bold text-zinc-200" data-testid={`admin-current-user-name-${instance}`}>{user?.name}</div><div className="truncate text-[11px] text-zinc-600" data-testid={`admin-current-role-name-${instance}`}>{user?.role_name || user?.role}</div></div>}<button type="button" onClick={logout} title="Keluar" className={`flex w-full items-center rounded-md py-2 text-sm font-semibold text-red-300 hover:bg-red-500/10 ${collapsed ? "justify-center px-2" : "gap-2 px-3"}`} data-testid={`admin-logout-${instance}`}><LogOut className="h-4 w-4" />{!collapsed && "Keluar"}</button></div>
  </div>;
};
