  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-zinc-400 mt-1">Pantau semua invoice (Pay Per Release + Annual Subscription). Mode MOCK.</p>
      </div>
      ...
      <div className="rm-card overflow-hidden">
        ...
        {items.length === 0 ? <div className="p-8 text-center text-zinc-500 text-sm">Belum ada invoice.</div> : items.map((i) => (
          <div key={i.id} className="px-5 py-4 grid grid-cols-12 gap-3 items-center border-b border-white/5 last:border-0">
            <div className="col-span-12 md:col-span-3 flex items-center gap-3 min-w-0">
              <div className="w-9 h-9 rounded-xl bg-white/[0.06] text-zinc-400 grid place-items-center"><CreditCard className="w-4 h-4" /></div>
              <div className="min-w-0">
                <div className="font-semibold text-sm truncate">{i.xendit_invoice_id}</div>
                <div className="text-xs text-zinc-500 truncate">{i.created_at?.slice(0, 16)?.replace("T", " ")}</div>
              </div>
            </div>
            ...
            <div className="col-span-6 md:col-span-2"><span className={`px-2.5 py-1 rounded-full text-xs font-bold capitalize ${i.status === "paid" ? "bg-emerald-500/15 text-emerald-300" : i.status === "pending" ? "bg-amber-500/100/15 text-amber-300" : "bg-white/[0.06] text-zinc-400"}`}>{i.status}</span></div>
