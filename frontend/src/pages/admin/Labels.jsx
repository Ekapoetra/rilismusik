  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rm-card overflow-hidden">
        ...
        {items.length === 0 ? <div className="p-8 text-center text-slate-500 text-sm">Belum ada label.</div> : items.map((l) => (
          <div key={l.id} className="px-5 py-4 grid grid-cols-12 gap-3 items-center border-b border-slate-50 last:border-0 hover:bg-slate-50/40">
            <div className="col-span-12 md:col-span-4">
              <div className="font-semibold">{l.label_name}</div>
              <div className="text-xs text-slate-500">{l.pic_name}</div>
            </div>
            ...
            <div className="col-span-6 md:col-span-2">
              <span className={`px-2.5 py-1 rounded-full text-xs font-bold capitalize ${l.account_status === "active" ? "bg-emerald-50 text-emerald-700" : l.account_status === "suspended" ? "bg-amber-50 text-amber-700" : "bg-red-50 text-red-700"}`}>{l.account_status}</span>
            </div>
            <div className="col-span-6 md:col-span-1 text-right">
              <Link to={`/admin/labels/${l.id}`} className="text-sm font-semibold text-[#FF3B30]" data-testid={`admin-label-detail-${l.id}`}>Detail →</Link>
            </div>
          </div>
        ))}
      </div>
