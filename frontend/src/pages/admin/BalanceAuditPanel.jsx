    ...
    <div className="space-y-5" data-testid="admin-balance-audit-panel">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-1 max-w-3xl">Bandingkan saldo tersimpan dengan royalty_lines setelah bulan withdraw terakhir. Label dengan withdraw aktif hanya ditampilkan dan tidak akan diubah.</p>
        </div>
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 border border-white/10 rounded-lg overflow-hidden" data-testid="admin-balance-audit-summary">
        ...
        <>
          ...
          {!audit.commitJob && summary.drift_labels > 0 && (
            <div className="border-y border-white/10 py-4 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
              <label className="flex gap-3 items-start text-sm text-zinc-300 cursor-pointer" data-testid="admin-balance-audit-confirm-label"><input type="checkbox" checked={audit.confirmed} onChange={(event) => audit.setConfirmed(event.target.checked)} className="mt-1 accent-emerald-400" data-testid="admin-balance-audit-confirm" /><span>Saya sudah memeriksa preview. Set saldo semua label tanpa withdraw aktif sesuai royalty_lines setelah withdraw terakhir, dan ubah baris lama ≤ cutoff menjadi withdrawn.</span></label>
      ...
      {commitDone && <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-4 py-4" data-testid="admin-balance-audit-complete"><div className="font-bold text-emerald-300 flex items-center gap-2"><CheckCircle2 className="w-4 h-4" /> Rekonsiliasi selesai</div><div className="grid sm:grid-cols-3 gap-3 mt-3 text-xs"><Result label="Label dikoreksi" value={audit.commitJob.result?.labels_reconciled} /><Result label="Baris lama diselesaikan" value={(audit.commitJob.result?.stale_lines_settled || 0).toLocaleString("id-ID")} /><Result label="Gagal" value={audit.commitJob.result?.labels_failed} /></div><button onClick={audit.startPreview} className="rm-btn-ghost mt-4 flex items-center gap-2" data-testid="admin-balance-audit-verify"><RefreshCw className="w-4 h-4" /> Audit ulang untuk verifikasi</button></div>}
    </div>
  );
}

function AuditRow({ row }) {
  const status = row.audit_status === "drift" ? ["Perlu koreksi", "text-amber-300 bg-amber-500/15"] : row.audit_status === "blocked_active_withdraw" ? ["Withdraw aktif", "text-sky-300 bg-sky-500/15"] : ["Sesuai", "text-emerald-300 bg-emerald-500/15"];
  return <tr className="border-t border-white/5" data-testid={`admin-balance-audit-row-${row.label_id}`}><td className="px-3 py-3 font-semibold">{row.label_name}</td><td className="px-3 py-3 font-mono">{fmtPeriod(row.last_withdrawn_period)}</td><td className="px-3 py-3 font-mono">{fmtPeriod(row.latest_report_period)}</td><td className="px-3 py-3 font-mono">{row.eligible_period_from ? `${row.eligible_period_from} → ${row.eligible_period_to}` : "Kosong"}</td><td className="px-3 py-3 text-right font-mono"><span className={row.current_pending_idr < 0 ? "text-red-300" : "text-zinc-400"}>{fmtIDR(row.current_pending_idr)}</span><span className="text-zinc-600 mx-1">→</span><span className="text-white">{fmtIDR(row.expected_pending_idr)}</span></td><td className="px-3 py-3 text-right font-mono"><span className="text-zinc-400">{fmtIDR(row.current_available_idr)}</span><span className="text-zinc-600 mx-1">→</span><span className="text-white">{fmtIDR(row.expected_available_idr)}</span></td><td className="px-3 py-3 text-right font-mono">{(row.stale_cutoff_lines || 0).toLocaleString("id-ID")}</td><td className="px-3 py-3"><span className={`px-2 py-1 rounded-full text-[10px] font-bold ${status[1]}`}>{status[0]}</span></td></tr>;
}
