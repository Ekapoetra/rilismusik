...
export default function BalanceAuditPanel({ label = null, onComplete }) {
  const audit = useBalanceAudit({ labelIds: label ? [label.id] : null });
  const notifiedJob = useRef(null);
  ...
  useEffect(() => {
    if (!commitDone || !audit.commitJob?.id || notifiedJob.current === audit.commitJob.id) return;
    notifiedJob.current = audit.commitJob.id;
    onComplete?.(audit.commitJob.result);
  }, [commitDone, audit.commitJob, onComplete]);

  return (
    <div className="space-y-5" data-testid="admin-balance-audit-panel">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="text-xs uppercase tracking-widest text-cyan-400 font-bold">Integritas Keuangan</div>
          <h3 className="font-display font-bold text-xl tracking-normal mt-1">{label ? `Audit & Sesuaikan Saldo ${label.label_name}` : "Audit Saldo Semua Label"}</h3>
          <p className="text-xs text-zinc-400 mt-1 max-w-3xl">{label ? `Hitung ulang baris setelah cutoff memakai persentase aktif ${Number(label.royalty_percentage_default || 60).toLocaleString("id-ID")}% dan kurs setiap import. Data sebelum cutoff serta withdrawal web tidak diubah.` : "Bandingkan saldo dengan laporan royalti, temukan data yang salah berstatus sudah ditarik, dan lindungi riwayat penarikan yang belum lengkap."}</p>
        </div>
        <button onClick={audit.startPreview} disabled={audit.busy} className="rm-btn-primary flex items-center gap-2" data-testid="admin-balance-audit-start">
          ...
          {previewDone ? "Hitung Ulang Preview" : label ? "Preview Penyesuaian" : "Mulai Preview Audit"}
        </button>
      </div>
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 border border-white/10 rounded-lg overflow-hidden" data-testid="admin-balance-audit-summary">
          ...
          <Summary label="Baris beda perhitungan" value={(summary.calculation_mismatch_lines || 0).toLocaleString("id-ID")} tone="text-cyan-300" testId="calculation-lines" />
          ...
          <Summary label="Data hitung legacy dipertahankan" value={(summary.calculation_missing_lines || 0).toLocaleString("id-ID")} tone="text-zinc-300" testId="calculation-missing" />
      ...
      {previewDone && (
        <>
          {!label && <div className="flex flex-wrap gap-3 items-end">
...
function AuditRow({ row }) {
  ...
  return <tr className="border-t border-white/5" data-testid={`admin-balance-audit-row-${row.label_id}`}><td className="px-3 py-3 font-semibold">{row.label_name}</td><td className="px-3 py-3 font-mono">{fmtPeriod(row.last_withdrawn_period)}</td><td className="px-3 py-3 font-mono">{fmtPeriod(row.effective_withdraw_cutoff)}</td><td className="px-3 py-3 font-mono">{fmtPeriod(row.latest_report_period)}</td><td className="px-3 py-3 font-mono">{row.eligible_period_from ? `${row.eligible_period_from} → ${row.eligible_period_to}` : "Kosong"}</td><td className="px-3 py-3 text-right font-mono"><span className={row.current_pending_idr < 0 ? "text-red-300" : "text-zinc-400"}>{fmtIDR(row.current_pending_idr)}</span><span className="text-zinc-600 mx-1">→</span><span className="text-white">{fmtIDR(row.expected_pending_idr)}</span></td><td className="px-3 py-3 text-right font-mono"><span className="text-zinc-400">{fmtIDR(row.current_available_idr)}</span><span className="text-zinc-600 mx-1">→</span><span className="text-white" data-testid={`admin-balance-audit-expected-available-${row.label_id}`}>{fmtIDR(row.expected_available_idr)}</span></td><td className="px-3 py-3 text-right font-mono"><div>{(row.calculation_mismatch_lines || 0).toLocaleString("id-ID")} baris</div><div><span className="text-zinc-500">{fmtIDR(row.calculation_mismatch_current_idr)}</span><span className="mx-1 text-zinc-600">→</span><span className="text-cyan-300">{fmtIDR(row.calculation_mismatch_projected_idr)}</span></div><div className="text-[10px] text-zinc-500">Bagian aktif {Number(row.royalty_percentage_current || 0).toLocaleString("id-ID")}%</div></td><td className="px-3 py-3 text-right font-mono"><div>{(row.wrongly_settled_lines || 0).toLocaleString("id-ID")} baris</div><div className="text-orange-300">{fmtIDR(row.wrongly_settled_idr)}</div>{row.orphan_legacy_settled_lines > 0 && <div className="text-[10px] text-zinc-500">{row.orphan_legacy_settled_lines.toLocaleString("id-ID")} bertanda sudah dibayar</div>}{row.import_status_mismatch_lines > 0 && <div className="text-[10px] text-zinc-500">{row.import_status_mismatch_lines.toLocaleString("id-ID")} belum mengikuti status laporan</div>}</td><td className="px-3 py-3 text-right font-mono">{(row.stale_cutoff_lines || 0).toLocaleString("id-ID")}</td><td className="px-3 py-3"><span className={`px-2 py-1 rounded-full text-[10px] font-bold ${status[1]}`}>{status[0]}</span></td></tr>;
}
