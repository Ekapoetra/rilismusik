...
import { Percent, Check, X, ExternalLink } from "lucide-react";
...
export default function RateChangeQueue() {
  ...
  const canApprove = hasPermission("labels.rate.approve");
  ...
  const load = useCallback(async () => {
    setErr("");
    try { const { data } = await api.get("/admin/rate-changes", { params: tab ? { status: tab } : {} }); setRows(data || []); }
  ...
  const decide = async (req, action) => {
    ...
    setBusy(true); setErr("");
    try {
      await api.post(`/admin/rate-changes/${req.id}/decision`, { action, note });
  ...
  return (
    <div className="space-y-6" data-testid="admin-rate-change-queue">
      <header className="border-b border-white/10 pb-5">
        ...
        <h1 className="mt-1 flex items-center gap-2 font-display text-3xl font-extrabold"><Percent className="h-6 w-6 text-pink-400" /> Permintaan Perubahan Rate/Fee</h1>
        <p className="mt-2 text-sm text-zinc-400">Nilai royalti label tidak berubah sampai Super Admin menyetujui permintaan.</p>
      </header>
      ...
      {rows.length === 0 ? (
        ...
        <div className="space-y-3">
          {rows.map((req) => (
            <div key={req.id} className="rounded-lg border border-white/10 bg-white/[0.02] p-4" data-testid={`rate-queue-row-${req.id}`}>
              ...
              <div className="mt-2 text-lg font-bold">{req.current_value}% → {req.proposed_value}%</div>
