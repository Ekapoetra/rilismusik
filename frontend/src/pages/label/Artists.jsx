    ...
    <div className="space-y-5 max-w-5xl">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        {items.length === 0 ? (
          ...
          <div key={a.id} className="rm-card p-5" data-testid={`label-artist-card-${a.id}`}>
            <div className="flex items-center gap-3">
              ...
              <div className="mt-3 rounded-xl bg-emerald-500/5 border border-emerald-500/15 px-3 py-2" data-testid={`label-artist-unwithdrawn-${a.id}`}>
                <div className="text-[9px] uppercase tracking-widest font-bold text-emerald-400/70">Royalti Belum Ditarik</div>
                <div className="mt-1 flex items-baseline justify-between gap-2">
                  <div className="font-mono font-bold text-emerald-300 text-sm" data-testid={`label-artist-unwithdrawn-value-${a.id}`}>{new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 }).format(a.revenue_idr || 0)}</div>
                  <div className="text-[10px] text-zinc-500">{a.last_active_period ? fmtPeriodInline(a.last_active_period) : "Tidak ada saldo"}</div>
                </div>
                <div className="text-[10px] text-zinc-500 mt-0.5">{(a.royalty_lines_count || 0).toLocaleString("id-ID")} baris pending/available</div>
              </div>
      ...
      {open && (
        <div className="fixed inset-0 z-50 bg-black/40 grid place-items-center p-4" onClick={() => setOpen(false)} data-testid="label-artist-create-modal">
          <form onClick={(e) => e.stopPropagation()} onSubmit={submit} className="w-full max-w-md rm-glass-strong rounded-[24px] p-6 space-y-4" data-testid="label-artist-create-form">
            <div className="flex justify-between items-center">
              <h3 className="font-display font-extrabold text-xl tracking-tighter">Tambah Artist</h3>
              <button type="button" onClick={() => setOpen(false)} data-testid="label-artist-create-close"><X className="w-5 h-5" /></button>
            </div>
            <div>
            ...
            <div>
              ...
              <input className="rm-input" value={form.whatsapp} onChange={(e) => setForm({ ...form, whatsapp: e.target.value })} data-testid="label-artist-whatsapp-input" />
            </div>
            <div>
            ...
            {err && <div className="text-sm text-red-600 bg-red-50 rounded-xl px-3 py-2" data-testid="label-artist-create-error">{err}</div>}
            <div className="flex justify-end gap-2">
              <button type="button" className="rm-btn-ghost" onClick={() => setOpen(false)} data-testid="label-artist-create-cancel">Batal</button>
