...
export const ReleaseInfoStep = ({ form, updateForm, labelName, responsibleName, onNext }) => {
  ...
  return <section className="space-y-7" data-testid="upload-release-info-step"><div><h2 className="font-display text-2xl font-bold">Informasi rilisan</h2><p className="mt-1 text-sm text-zinc-400">Data utama yang tampil di DSP dan dokumen distribusi. Kolom bertanda opsional boleh dikosongkan.</p></div><div className="grid gap-5 md:grid-cols-2">
    ...
    <Field label="Tipe Rilisan"><select className="rm-input" value={form.release_type} onChange={change("release_type")} data-testid="upload-release-type-select"><option value="single">SINGLE</option><option value="ep">EP</option><option value="album">ALBUM</option></select><p className="mt-1 text-xs text-zinc-500" data-testid="upload-release-type-hint">SINGLE = 1 lagu · EP = 2–6 lagu · ALBUM = 7–12 lagu</p></Field>
