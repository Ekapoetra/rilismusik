...
export default function LabelWithdraw() {
  ...
  if (!window || !dash) return <div className="text-slate-500">Memuat…</div>;
  ...
  return (
    <div className="space-y-5 max-w-3xl">
      <div>
        <div className="text-xs uppercase tracking-widest text-slate-500 font-bold">Withdraw</div>
        <h1 className="font-display text-3xl font-extrabold tracking-tighter">Tarik Saldo</h1>
        <p className="text-sm text-slate-600 mt-1">Permintaan withdraw tanggal 1-14, pembayaran 15-20.</p>
      </div>

      <div className={`rounded-2xl p-4 text-sm flex items-start gap-3 ${window.request_open ? "bg-emerald-50 text-emerald-800" : window.payment_window ? "bg-sky-50 text-sky-800" : "bg-amber-50 text-amber-800"}`}>
      ...
      {!hasBank ? (
        <div className="rm-card p-6 text-center">
          ...
          <p className="text-sm text-slate-600 mt-1">Hubungi admin untuk verifikasi atau input rekening dulu.</p>
      ...
      <div className="rm-card p-5">
        ...
        {items.length === 0 ? <div className="text-sm text-slate-500 py-6 text-center">Belum ada withdraw.</div> : items.map((w) => (
          <div key={w.id} className="py-3 border-b border-slate-50 last:border-0 flex items-center justify-between flex-wrap gap-3">
            <div>
              <div className="font-display font-bold text-lg tracking-tight">{fmtIDR(w.amount_idr)}</div>
              <div className="text-xs text-slate-500">Request {w.request_date?.slice(0, 10)} {w.paid_date && `• Dibayar ${w.paid_date.slice(0, 10)}`}</div>
              {w.admin_note && <div className="text-xs text-amber-700 mt-1">Catatan: {w.admin_note}</div>}
            </div>
            <span className={`px-2.5 py-1 rounded-full text-xs font-bold capitalize ${STATUS_PILL[w.status] || "bg-slate-100 text-slate-600"}`}>{w.status}</span>
          </div>
        ))}
      </div>
    </div>
...
function Card({ label, v, accent }) {
  const cls = { emerald: "text-emerald-700", amber: "text-amber-700", sky: "text-sky-700" }[accent] || "";
  return <div className="rm-card p-4"><div className="text-[10px] uppercase tracking-widest text-slate-500 font-bold">{label}</div><div className={`font-display text-xl md:text-2xl font-extrabold tracking-tighter mt-1 ${cls}`}>{v}</div></div>;
}
